                        ...
                        using (OdbcDataReader reader = command.ExecuteReader())
                        {
                            if (reader.HasRows)
                            {
                                ...
                                while (reader.Read())
                                {
                                    ...
                                    ccPallet.BestBefore = bestBeforeDate.ToString(DateFormats.ddMMyy);
                                    ccPallet.OldBestBefore = bestBeforeDate.ToString(DateFormats.ddMMyy);
                        ...
                        if ((pallets[i].OldBestBefore != pallets[i].BestBefore) || (pallets[i].OldStockQuantity != pallets[i].StockQuantity))
                        {
                            ...
                            else
                            {
                                ...
                                using (OdbcCommand command = new OdbcCommand(updateString, connection))
                                {
                                    ...
                                    if (!String.IsNullOrEmpty(pallets[i].BestBefore))
                                    {
                                        command.Parameters.Add("@BestBefore", OdbcType.VarChar).Value = DateTime.Parse(pallets[i].BestBefore, new CultureInfo("en-AU")).ToString(DateFormats.MMddyy);
                                    }
                                    else
                                    {
                                        command.Parameters.Add("@BestBefore", OdbcType.VarChar).Value = "";
                                    ...
                                    {
                                        command.Parameters.Add("@OldBestBefore", OdbcType.VarChar).Value = DateTime.Parse(pallets[i].OldBestBefore, new CultureInfo("en-AU")).ToString(DateFormats.MMddyy);
                                    }
                                    else
                                    {
